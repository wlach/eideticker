...
def main(args=sys.argv[1:]):
    usage = "usage: %prog [options] <output dir>"
    ...
    parser.add_option("--test", "-t",
                      ...
                      help="Restrict testing to specific test (options: %s)" %
                      ", ".join([test["name"] for test in default_tests]))
    ...
    tests = default_tests
    if options.test:
        tests = [test for test in tests if test['name'] == options.test]
        if not tests:
            print "ERROR: No tests matching '%s'" % options.test
            sys.exit(1)

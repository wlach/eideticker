...
class OptionParser(optparse.OptionParser):
    '''Custom version of the optionparser class'''
    '''with a few videocapture-specific parameters'''
    ...
    def _add_options(self):
        self.add_option("--capture-device", action="store",
                        ...
                        help="type of capture device (%s)" % " or ".join(
                            valid_capture_devices))
        self.add_option("--mode", action="store",
                        type="string", dest="mode",
                        default=None,
                        help="mode to use with decklink cards (%s)" %
                        " or ".join(valid_decklink_modes))
        ...
        if self.capture_area_option:
            self.add_option("--capture-area", action="store",
                            ...
                            help="Hardcode capture area. Must be passed in as "
    ...
    def parse_args(self):
        ...
        if options.capture_device not in valid_capture_devices:
            self.error("Capture device must be %s" % " or ".join(
                valid_capture_devices))
        ...
        if self.capture_area_option and options.capture_area:
            try:
                ...
                if not isinstance(capture_area, list) or \
                        len(capture_area) != 4 or \
